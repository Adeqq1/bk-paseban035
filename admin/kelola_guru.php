...
if (!isset($_SESSION['role']) || $_SESSION['role'] !== 'admin') {
    header("Location: ../index.php"); // Jika pengguna belum login/bukan admin, alihkan ke login
    exit();
}

// =========================================================================
// 2. PROSES TAMBAH DATA WALI KELAS BARU & PEMBUATAN AKUN AUTOMATIS
// Menambahkan profil wali kelas sekaligus membuat akun login di tabel `user`.
// =========================================================================
if (isset($_POST['tambah'])) {
    ...
    // Username dan password bawaan (default) menggunakan NIP yang dienkripsi
    $username = $nip; 
    ...
    if (mysqli_num_rows($cek_nip) > 0 || mysqli_num_rows($cek_user) > 0) {
        ...
        // Memulai Transaksi Database agar kedua tabel tersimpan secara bersamaan
        mysqli_begin_transaction($koneksi);

        try {
            // A. Tambahkan data akun ke tabel `user`
            ...
            $user_id = mysqli_insert_id($koneksi); // Ambil ID user yang baru terbuat

            // B. Tambahkan data profil ke tabel `guru` terhubung dengan user_id
            $query_guru = "INSERT INTO guru (user_id, nip, nama_lengkap, jabatan) 
                            VALUES ('$user_id', '$nip', '$nama', '$jabatan')";
            ...
            // Jika terjadi kegagalan, batalkan seluruh perubahan
...
// =========================================================================
// 3. PROSES EDIT / PERBARUI DATA WALI KELAS & AKUN SISTEM
// Mengubah data NIP, Nama Lengkap, atau Jabatan pada tabel guru dan user.
// =========================================================================
if (isset($_POST['edit'])) {
    ...
    // Cari ID User yang terhubung dengan ID Guru
    ...
    try {
        // A. Perbarui username (NIP) dan role di tabel `user`
        $query_user = "UPDATE user SET username='$nip', role='$role' WHERE id='$user_id'";
        mysqli_query($koneksi, $query_user);

        // B. Perbarui profil Wali Kelas di tabel `guru`
        $query_guru = "UPDATE guru SET nip='$nip', nama_lengkap='$nama', jabatan='$jabatan' WHERE id='$id'";
...
// =========================================================================
// 4. PROSES HAPUS DATA WALI KELAS
// Menghapus data akun user (secara otomatis menghapus profil guru via CASCADE).
// =========================================================================
if (isset($_GET['hapus'])) {
    $id = mysqli_real_escape_string($koneksi, $_GET['hapus']);
    
    // Ambil ID User terlebih dahulu
    $res = mysqli_query($koneksi, "SELECT user_id FROM guru WHERE id='$id'");
    if ($row = mysqli_fetch_assoc($res)) {
        $user_id = $row['user_id'];

        // Hapus dari tabel user (otomatis menghapus data di tabel guru)
...
// =========================================================================
// 5. PROSES RESET KATA SANDI WALI KELAS KE DEFAULT (NIP)
// Mengembalikan password dan username akun Wali Kelas kembali ke NIP awalnya.
// =========================================================================
if (isset($_GET['reset_sandi'])) {
    ...
    if ($row = mysqli_fetch_assoc($res)) {
        ...
        // Enkripsi ulang kata sandi default (NIP)
...
// =========================================================================
// 6. QUERY PENAMPILAN DATA & FILTER PENCARIAN WALI KELAS
// Mengambil daftar Wali Kelas dan gabungan (LEFT JOIN) nama kelas yang diampu.
// =========================================================================
$search_query = $_GET['search'] ?? '';
$where_sql = "WHERE g.jabatan='Wali Kelas'";
if (!empty($search_query)) {
    $search_safe = mysqli_real_escape_string($koneksi, $search_query);
    $where_sql .= " AND (g.nama_lengkap LIKE '%$search_safe%' OR g.nip LIKE '%$search_safe%' OR k.nama_kelas LIKE '%$search_safe%')";
}

// Jalankan query SQL penarikan data
...
<head>
    ...
    <title>Kelola Wali Kelas | BK SMA 07 Bungo</title>
    <meta name="description" content="Manajemen data Wali Kelas - BK SMA 07 Bungo">
...
<body>
    ...
    <div class="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li><a href="kelola_guru.php" class="active"><i class="fas fa-chalkboard-teacher"></i> Kelola Wali Kelas</a></li>
    ...
    <!-- =================================================================== -->
    <!-- KONTEN UTAMA HALAMAN KELOLA WALI KELAS                               -->
    <!-- =================================================================== -->
    <div class="main-content">
        ...
        <div class="header" style="background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); padding: 2rem; border-radius: 16px; margin-bottom: 2rem; color: white; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1rem; box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.3); border: 1px solid rgba(255,255,255,0.05); position: relative; overflow: hidden;">
            ...
            <div style="display: flex; align-items: center; gap: 1.5rem; position: relative; z-index: 1;">
                <div style="background: rgba(255,255,255,0.06); width: 60px; height: 60px; border-radius: 12px; display: flex; align-items: center; justify-content: center; border: 1px solid rgba(255,255,255,0.1); box-shadow: inset 0 2px 4px rgba(255,255,255,0.05);">
                    <i class="fas fa-chalkboard-teacher" style="font-size: 1.8rem; color: #60a5fa;"></i>
                </div>
                <div>
                    <h1 style="margin: 0 0 6px 0; font-size: 1.6rem; font-weight: 800; color: white; letter-spacing: -0.01em;">Kelola Wali Kelas</h1>
                    <p style="margin: 0; color: #94a3b8; font-size: 0.925rem;">Manajemen data dan akun seluruh Wali Kelas.</p>
                </div>
            </div>
            <!-- Tombol Pemicu Modal Tambah Wali Kelas -->
            <button class="btn-tambah-utama" onclick="openModal('modalTambah')" id="btnTambahWali" style="position: relative; z-index: 1;">
        ...
        <?php if (isset($msg)): ?>
            <div class="alert <?php echo $msg == 'success_hapus' ? 'alert-delete' : (strpos($msg, 'success') !== false ? 'alert-success' : 'alert-danger'); ?>">
                ...
                <?php 
                    if ($msg == 'success_tambah') echo "Data Wali Kelas berhasil ditambahkan!";
                    if ($msg == 'success_edit') echo "Data Wali Kelas berhasil diperbarui!";
                    if ($msg == 'success_hapus') echo "Data Wali Kelas berhasil dihapus!";
                    if ($msg == 'success_reset') echo "Akun (Username & Sandi) Wali Kelas berhasil direset ke NIP dan email pemulihan dikosongkan.";
                    if ($msg == 'error_duplikat') echo "NIP tersebut sudah terdaftar di sistem!";
                    if ($msg == 'error_not_found') echo "Data guru tidak ditemukan.";
                    if ($msg == 'error') echo "Terjadi kesalahan sistem.";
                ?>
            </div>
        <?php endif; ?>

        <!-- Kartu Tabel Data Wali Kelas & Form Filter Pencarian -->
        <div class="data-card">
            <div class="data-card-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
                <h2 style="margin: 0;"><i class="fas fa-chalkboard-teacher"></i> Daftar Wali Kelas</h2>
                
                <!-- Form Filter Pencarian NIP / Nama -->
                <form method="GET" action="" style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">
                    <input type="text" name="search" class="form-control" placeholder="Cari nama, NIP, atau kelas..." value="<?php echo htmlspecialchars($search_query); ?>" style="width: 250px; padding: 8px 12px; font-size: 0.85rem; height: 38px; border-radius: 8px; border: 1px solid #e2e8f0; outline: none;">
                    ...
                    <?php if(!empty($search_query)): ?>
                        <a href="kelola_guru.php" class="btn" style="background: #e2e8f0; color: #475569; padding: 8px 16px; font-size: 0.85rem; height: 38px; display: inline-flex; align-items: center; text-decoration: none; border-radius: 8px;">
                            Reset
                        </a>
                    <?php endif; ?>
                </form>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <!-- Kolom 2: NIP Wali Kelas -->
                            <td><small style="color: #475569; font-weight: 400;"><?php echo htmlspecialchars($row['nip'] ?? '-'); ?></small></td>
                            
                            <!-- Kolom 3: Nama Lengkap Wali Kelas -->
                            <td><span style="color: #334155; font-weight: 400;"><?php echo htmlspecialchars($row['nama_lengkap']); ?></span></td>
                            
                            <!-- Kolom 4: Status Pengampuan Kelas -->
                            <td>
                                <?php if($row['nama_kelas']): ?>
                                    <span class="badge badge-info"><?php echo $row['nama_kelas']; ?></span>
                                <?php else: ?>
                                    <span style="color:var(--text-muted);font-style:italic;font-size:.82rem;">Belum ditugaskan</span>
                                <?php endif; ?>
                            </td>
                            
                            <!-- Kolom 5: Tombol Aksi (Edit, Reset Sandi, Hapus) -->
                            <td style="text-align: center; white-space: nowrap;">
                                <div style="display: flex; gap: 6px; justify-content: center;">
                                    <!-- Tombol Edit Data -->
                                    <button class="btn btn-warning btn-sm btn-icon" onclick='editGuru(<?php echo htmlspecialchars(json_encode($row), ENT_QUOTES, "UTF-8"); ?>)' title="Edit Data">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    
                                    <!-- Tombol Reset Password ke Default (NIP) -->
                                    <a href="?reset_sandi=<?php echo $row['id']; ?>" class="btn btn-primary btn-sm btn-icon" onclick="return confirm('Reset sandi Wali Kelas ini ke NIP?')" title="Reset Sandi ke NIP">
                                        <i class="fas fa-key"></i>
                                    </a>
                                    
                                    <!-- Tombol Hapus Data Wali Kelas -->
                                    <a href="?hapus=<?php echo $row['id']; ?>" class="btn btn-danger btn-sm btn-icon" onclick="return confirm('Apakah Anda yakin ingin menghapus data guru ini?')" title="Hapus Data">
    ...
    <!-- =================================================================== -->
    <!-- MODAL DIALOG POPUP: TAMBAH WALI KELAS BARU                           -->
    <!-- =================================================================== -->
    <div id="modalTambah" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <div>
                    <h2 style="font-size: 1.25rem; font-weight: 700; color: #1e293b; margin: 0 0 4px 0;">Tambah Wali Kelas Baru</h2>
                    <p style="font-size: 0.85rem; color: #64748b; margin: 0;">Password default adalah NIP Wali Kelas.</p>
                </div>
                <div class="close" onclick="closeModal('modalTambah')">&#x2715;</div>
            </div>
            <!-- Form Input Data Baru -->
            <form action="kelola_guru.php" method="POST">
                ...
                <div class="form-group" style="margin-bottom: 1.5rem;">
                    ...
                    <input type="text" name="jabatan" class="form-control" value="Wali Kelas" readonly style="background-color: #f1f5f9; cursor: not-allowed;">
                </div>
    ...
    <!-- =================================================================== -->
    <!-- MODAL DIALOG POPUP: EDIT DATA WALI KELAS                             -->
    <!-- =================================================================== -->
    <div id="modalEdit" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <div>
                    <h2 style="font-size: 1.25rem; font-weight: 700; color: #1e293b; margin: 0 0 4px 0;">Edit Data Wali Kelas</h2>
                    <p style="font-size: 0.85rem; color: #64748b; margin: 0;">Perbarui informasi Wali Kelas.</p>
                </div>
            ...
            <form action="kelola_guru.php" method="POST">
                ...
                <div class="form-group" style="margin-bottom: 1.5rem;">
                    ...
                    <input type="text" name="jabatan" id="edit_jabatan" class="form-control" value="Wali Kelas" readonly style="background-color: #f1f5f9; cursor: not-allowed;">
                </div>
                <div class="modal-footer" style="margin-top: 2rem;">
    ...
    <!-- =================================================================== -->
    <script>
        // Fungsi untuk membuka modal dialog berdasarkan ID
        function openModal(id) {
            document.getElementById(id).style.display = 'flex';
        }

        // Fungsi untuk menutup modal dialog berdasarkan ID
        function closeModal(id) {
            document.getElementById(id).style.display = 'none';
        }

        // Fungsi untuk mengisi otomatis field form edit dari data objek guru
        function editGuru(guru) {
            document.getElementById('edit_id').value = guru.id;
            document.getElementById('edit_nip').value = guru.nip;
            document.getElementById('edit_nama').value = guru.nama_lengkap;
            document.getElementById('edit_jabatan').value = guru.jabatan || 'Wali Kelas';
            openModal('modalEdit');
        }

        // Menutup modal jika pengguna mengklik area luar modal dialog
